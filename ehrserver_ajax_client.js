var ehrserver_ajax_client = {
   ...
   queries: function (max, offset, callback) {
     max = (typeof max !== 'undefined') ?  max : 20;
     offset = (typeof offset !== 'undefined') ?  offset : 0;
     $.ajax({
   ...
   query_execute: function (query_uid, ehr_uid, callback) {
      console.log(this.base_url + this.get_query_execute +'/'+ query_uid +'/execute');
     $.ajax({
       url: this.base_url + this.get_query_execute +'/'+ query_uid +'/execute',
       method: 'GET',
       beforeSend: function(xhr) {
         xhr.setRequestHeader('Authorization', 'Bearer '+ ehrserver_ajax_client.token);
       },
       data: {format: 'json', ehrUid: ehr_uid}
     })
     .done( function (data, textStatus, jqXHR) {
       callback(data, jqXHR.status, jqXHR.statusText);
     })
